...
are required. It knows nothing about tabsim's caching, cache-key policy, or
source-precedence policy — those live in :mod:`tabsim.satchecker.cache` and
:mod:`tabsim.orbit`. It deliberately imports nothing from tabsim, JAX, or
...
:mod:`tabsim.satchecker.service`'s decision, not this module's.
...
USER_AGENT = "tabsim-tle/1.0 (+https://github.com/chrisfinlay/tab-sim)"
...
class SatCheckerTransportError(SatCheckerError):
    ...
    same unreachable host. :func:`tabsim.satchecker.service.fetch_nearest_batch`
...
class SatCheckerRateLimitError(SatCheckerTransportError):
    ...
    it sends one, so the message can say when the run is worth repeating. tabsim
...
def _normalise_omm(records: pd.DataFrame) -> pd.DataFrame:
    ...
    :mod:`tabsim.satchecker.records`, which is also where the reasoning about
    that gap lives.
    """
...
def fetch_nearest_omm(norad_id: int, epoch_jd: float) -> pd.DataFrame:
    ...
    may be years off. The age ceiling in :mod:`tabsim.orbit` is what rejects it.
    """
