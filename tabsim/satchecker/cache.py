...
observation gets is decided by epoch distance in :mod:`tabsim.orbit`, not here.
        ...
        try:
            ...
            if envelope.get("schema_version") != SCHEMA_VERSION:
                raise CacheValidationError(
                    ...
                    f"(this tabsim writes {SCHEMA_VERSION})"
                )
...
def read_legacy_tle_records(directory) -> pd.DataFrame:
    ...
    for path in sorted(glob(str(directory / "*.json"))):
        try:
            ...
            # rounded — it reads 0.0066635 back as 0.006663499999999999, a
            # different double. For an OMM record that is the eccentricity, and
            # the replayed trajectory then disagrees with the run the file was
            # written to reproduce. Deviation from the vendored upstream; see
            # tabsim/satchecker/__init__.py.
