...
def main():
    ...
    parser.add_argument('--base-profile',default='default')
    parser.add_argument('--candidate-profile',default='default')
    ...
    for index in range(args.rounds):
        ...
        for label, root in targets:
            ...
            command = [args.python, str(Path(__file__).with_name('output_worker.py')),
                       '--root', root, '--output', str(target), '--device', args.device,
                       '--gpu-slots',str(args.base_slots if label=='parent' else args.candidate_slots),
                       '--profile',args.base_profile if label=='parent' else args.candidate_profile]
            process = None
