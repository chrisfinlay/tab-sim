...
output-I/O timing without accounting for the separate component stores.
"""
...
def pytest_configure(config):
    ...
    def staged_simulation(case, mode, chunk_mb, directory, progress=None):
        ...
        required = 2 * estimates(case, mode, chunk_mb)['disk_plan_bytes']
        if required > disk.free - max(2 * 2**30, .05 * disk.total):
            pytest.skip('Staged experiment needs space for components and complete output')
        start = time.perf_counter()
        if progress:
            progress('setup_start', {'experiment': 'staged-zarr-v1'})
        ...
        sizes = {name: sum(p.stat().st_size for p in (Path(directory) / name).rglob('*') if p.is_file())
                 for name in ('components', 'result.zarr')}
        return {'setup_s': setup-start, 'graph_build_s': graph-setup,
                ...
                'experimental_output_bytes': sizes, 'experiment': 'staged-zarr-v1',
